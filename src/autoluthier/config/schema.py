...
class RecordingConfig(BaseModel):
    ...
    Must exactly match the autoluthier MIDI session used to render ``sustain.wav`` (and
...
class StageConfig(BaseModel):
    ...
    alongside each stage under ``autoluthier.dsp`` and registered in ``dsp/registry.py``
