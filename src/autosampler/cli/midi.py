"""``autosampler midi`` — generate an autosampler MIDI session and its project.toml."""
...
def midi_command(
    *,
    ...
    """Write a MIDI autosampler session plus a matching project.toml.

    Next steps after this: load the ``.mid`` file into a DAW/plugin, render it to
    ``sustain.wav``, save it next to ``project.toml``, then run ``autosampler run``.
    """
    try:
    ...
    console.print(
        ...
        f"{result.project_path.parent / 'sustain.wav'}, then run 'autosampler run "
