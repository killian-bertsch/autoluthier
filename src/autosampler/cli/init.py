"""``autosampler init`` — scaffold a project.toml for a folder that already has a render."""
...
from autosampler.config.toml_io import PROJECT_FILENAME, save_project
...
def init_command(
    ...
    *,
    ...
    Use this when ``sustain.wav``/``.flac`` already exists (from `autosampler concat` or an
    external recording) and only needs a project.toml to be processed. For a brand-new session
    that also needs a MIDI file, use ``autosampler midi`` instead.
    """
