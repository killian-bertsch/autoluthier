...
point the server at an isolated index instead of a real user's ``~/.autosampler``.
"""
...
from autosampler.config.workspace import default_workspace_path
from autosampler.server.errors import install_error_handlers
from autosampler.server.routes import (
    analysis,
    events,
    helpers,
    jobs,
...
from autosampler.server.static import mount_frontend

FRONTEND_DIR = Path(__file__).resolve().parents[3] / "frontend"
"""Repo-root ``frontend/`` — three levels up from ``src/autosampler/server/app.py``."""
...
def create_app(*, workspace_path: Path | None = None) -> FastAPI:
    ...
    """
    app = FastAPI(title="autosampler", version="2.0.0", lifespan=_lifespan)
    app.state.autosampler = AppState(workspace_path or default_workspace_path())
