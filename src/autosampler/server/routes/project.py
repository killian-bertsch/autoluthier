...
from autosampler.config.toml_io import load_project as load_project_toml
from autosampler.config.toml_io import save_project
from autosampler.config.workspace import load_workspace, save_workspace
from autosampler.pipeline.executor import load_instrument_audio
from autosampler.server.state import AppState, LoadedInstrument
...
def open_project(body: OpenProjectRequest, request: Request) -> ProjectSummary:
    ...
    Raises:
        autosampler.config.toml_io.ProjectConfigError: mapped to 400 on a missing or
            invalid project.toml.
        autosampler.pipeline.executor.PipelineError: mapped to 400 if the recording can't
            be loaded.
    """
...
def save_and_reload(config: ProjectConfig, request: Request) -> ProjectSummary:
    ...
    Raises:
        autosampler.server.state.SessionError: mapped to 404 if no instrument is open.
        autosampler.pipeline.executor.PipelineError: mapped to 400 if the reload fails.
    """
